...
CodeMirror.keys = {
    Backspace: function (cm) {
        commands.deleteCharBackward(cm.cm6);
    },
    Delete: function (cm) {
        commands.deleteCharForward(cm.cm6);
    }
};
CodeMirror.keyMap = {};
...
function findMatchingTag(cm, pos) {
    ...
    var m = language.matchBrackets(state, offset + 1, -1, { brackets: "\n\n" });
    if (m) {
        if (!m.end || !m.start)
            return;
        return {
    ...
    m = language.matchBrackets(state, offset, 1, { brackets: "\n\n" });
    if (m) {
        if (!m.end || !m.start)
            return;
        return {
...
function findEnclosingTag(cm, pos) {
    ...
    var i = offset;
    while (i > 0) {
        var m = language.matchBrackets(state, i, 1, { brackets: "\n\n" });
        if (m && m.start && m.end) {
            return {
                open: convertRange(state.doc, m.start),
                close: convertRange(state.doc, m.end),
